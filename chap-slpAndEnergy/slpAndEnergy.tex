...
One of the main drawback leading to the surge of the number of variables is the arc symmetries.
By duplicating the stock arcs during the graph transformation, and then defining the flow affectation variables, we created multiple equivalent situations.
In practice, it is not necessary to create a flow affectation variable to each pairs of successive stock arcs.
Perhaps the best way to understand the reduction is via an example.

\begin{figure}[t]
\begin{center}
\scalebox{1}{\input{tikz/symetryExample}}
\end{center}
...
\medskip
Let consider the situation depicted in Figure \ref{fig:symetryExample}, where it is assumed that the arcs belong to the set of stock arcs ($\tegArcSet_1$).
Until then, four flow affectation variables were considered: $A_{ab}$, $A_{ad}$, $A_{cb}$, $A_{cd}$.
From a system point of view, a flow passing through an arc coming from node $x$, passing through $y$ and then going to node $z$ means that a vehicle is parked in the station during two time-steps.
This situation corresponds to the cases where one of the flow affectation variable enumerated above is set to $1$.
However, there is no difference in term of interpretation for a flow entering node y between continuing to arc $b$ or continuing to arc $d$.
...
With this idea, the number of flow affectation variables related to stock arcs decreases from $|\Gamma^-(x) \cap \tegArcSet_1|\cdot|\Gamma^+(x) \cap \tegArcSet_1| = Z(\eta(x))^2$ to $Z(\eta(x))$ for all $x \in \tegNodeSet$.
...
\subsection{Rounding solutions and heuristic}
