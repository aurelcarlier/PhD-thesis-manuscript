...
At any intermediate step of the algorithm, the energy label of a node $i$ is the maximum battery level of a vehicle performing the trip from the source.
...
\item $t \in \timeStepSet$ defining $\tegNodeSet_t = \{x \in \tegNodeSet \mid \theta (x) = t\}$: a set of departure nodes.
\end{itemize}}
...
The algorithm permanently labels the nodes $3$, $5$, $6$ and $2$ in this given sequence.
Then, nodes $1$, $4$ and $7$ are equivalent choices since they all admit a distance label $d(i) = 2$.
Because the source node (node $1$) can now pretend to be permanently labeled, the cycle search routine can ends in this iteration.
As a result, the shortest cycle is $(1, 3, 5, 6, 1)$ and satisfied two demands.
Energy constraints are 

 shows a simple instance to be familiar with the algorithm routine.
