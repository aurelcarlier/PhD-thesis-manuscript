...
To deal with this shortest path problem, we chose to use the Dijkstra's algorithm, a label-setting algorithm returning a solution in a polynomial time.
However, the path may not be feasible according to energy constraints and some algorithmic adjustments are needed.
It should be noted here that we actually are looking for cycles, due to the Theorem \ref{th:decomp} (page \pageref{th:decomp}).

\medskip
In addition to the standard distance label value, we associate to each node $i$ an energy label $e(i)$ standing for the vehicle battery level at this node, which is an lower bound of the energy remaining in the battery at this specific node.
...
At any intermediate step, the energy label of a node $i$ is the maximum battery level of a vehicle performing the trip from the source.
...
The algorithm terminates when no more cycle can be found.
It should be noted that the problem we address here is not to find shorter paths from a source node to every other nodes in the network.
The problematic is more to find shortest cycles, which can be viewed as paths starting and ending at the same vertex.
To solve the problem faster, we could terminate the algorithm as soon as it has selected the departure node from the list of temporary labelled nodes, even though some nodes are still in this list.

\medskip
Algorithm \ref{algo:dijEnergy} gives a formal algorithmic description of the modified Dijkstra's algorithm.

\begin{algorithm}
%\DontPrintSemicolon % Some LaTeX compilers require you to use \dontprintsemicolon instead
\KwIn{
\begin{itemize}
\item $\teg = (\tegNodeSet, \tegArcSet, e)$, a binary weighted TEG where weights are set to 0 on demands arcs and 1 elsewhere;
\item A set of departure nodes $\tegNodeSet_1 = \{x \in \tegNodeSet \mid \theta (x) = 1\}$.
\end{itemize}}
...
\While{$\tegNodeSet_1 \not= \emptyset$}{
	...
	$d(x) \leftarrow +\infty$ and $e(x) \leftarrow 0$, for each node $ x \in \bar{S}$\;
	...
	\medskip
	\tcp{Begin the main loop}
	\While{$\bar{S} \neq \emptyset$ {\bf or} $s \in \bar{S}$}{
	...
	\medskip
	\tcp{update sets before iterate}
	$cycle(s) \leftarrow $ the shortest cycle given by the directed out-tree\; % maintained with pred(i) $\forall i \in \tegNodeSet$\;
	\lIf{$\tegArcSet_2 \cap cycle(s) \not= \emptyset$}{$\tegArcSet \leftarrow \tegArcSet - \{cycle(s)\}$}	
	$\tegNodeSet_1 \leftarrow \tegNodeSet_1 - \{s\}$\;	
}
...
\caption{Illustrating the modified Dijkstra's algorithm considering energy components.}
\label{fig:heuristicExample}
\end{figure}

Figure \ref{fig:heuristicExample} shows a simple instance to be familiar with the algorithm routine.
