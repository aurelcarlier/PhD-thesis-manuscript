...
They are mainly intended for occasional drivers whom benefit from the use of a vehicle (a car) without both the necessity of buying a personal one and the responsibilities of ownership \cite{shaheen_carsharing_1998} as insurance, maintenance, fuel (or electricity), taxes, depreciation, etc.
Nowadays, there are many implementations of carsharing systems adopting many different forms.
However, they differ and have to be distinguished from traditional auto-rental services mainly because they are oriented to short-term rentals and fuel costs are included in the rental fee.
Although there are plenty of formal definition in the literature, one of the most concise could be the one given by Millard-Ball et al. in (2005) \cite{millard_ball_car_sharing_2005} which defines carshaing as
\begin{quote}
...
\begin{quote}
``Carsharing is the pooling of an inland transport motor vehicles fleet for the benefit of subscribers. Each one them can access a driverless vehicle for the ride of his choice for a limited time.''
\end{quote}

The first experiment is identified in \cite{shaheen_short_1999} as Sefage, a Swiss company created in 1948.
...
Nowadays, we can find over the world three different types of carsharing systems called respectively 'Round-trip', 'One-way' and 'Free-floating' systems.
