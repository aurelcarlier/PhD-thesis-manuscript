...
Then, a monthly membership based on the time spent on roads and the distance travelled testifies to the personal usage of each member.

\medskip
The technology provided by the operator can vary enormously but generally, members have access to vehicles on-the-go.
...
Vehicles are dispersed all across the city through many indicated stations.
Station-based systems can be formally defined as \cite{shaheen_short_1999} follows:
\begin{quote}
...
\end{quote}
Generally, they dispose of their own dedicated parking lots and provide additional specific infrastructure if necessary.
The carsharing service Autolib' in Paris, France for instance offers charging points for electric vehicles and kiosks for customer service.

% whereas the third one, appeared recently, offers more flexibility leaving the choice to the user to leave a vehicle where he wants.
...
Although the first of them have been station-based designed, we have witnessed in the last decade the emergence of carsharing models without stations.
...
Moreover, carsharing reduces the time spent at finding a parking lot, which is often important in dense urban areas.
...
%It services offers many benefits to various actors.
 

Different studies evaluated that carsharing could be a real alternative to private car.
With respect to local costs, a car user driving less than $10,000$ kilometres per year (as much as $15,000$ km/y) could save money using carsharing \cite{litman_evaluating_2000, prettenthaler_ownership_1999}.
