...
%With the inputs and outputs listed above, we present in this section a formal definition of the System Dimensioning Problem.
%It can be expressed as follows:
%~\\
%
%{\noindent\textbf{System Dimensioning Problem [\textsc{SDP}]:}}
%\begin{description}
%\item[Inputs:]
%$\timeStepSet$, $\stationSet$, $\sCapaSet$, $\demandSet$ and $\travelTimeSet$.
%\item[Question:]
%What is the optimal system configuration allowing to capture the $\nbDemands$ demands ?
%\end{description}
...
\sum_{\substack{y\in \Gamma^-(x)}} \varphi((y,x)) = \sum_{\substack{y\in \Gamma^+(x)}} \varphi((x,y)) &$\forall x \in \tegNodeSet$\\
